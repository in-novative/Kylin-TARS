...
monitor_start_btn = None
monitor_start_output = None
...
try:
    ...
    print("[INFO] MonitorAgent 和 MediaAgent 导入成功")
except ImportError as e:
    HAS_MONITOR_AGENT = False
    HAS_MEDIA_AGENT = False
    print(f"[WARNING] MonitorAgent或MediaAgent未找到，相关功能不可用。错误: {e}")
                ...
                with gr.Accordion("⚙️ SettingsAgent - 系统设置", open=False):
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            gr.Markdown("#### 蓝牙管理")
                            bluetooth_action = gr.Radio(label="操作", choices=["status", "enable", "disable", "connect", "list"], value="status", interactive=True)
...
{result.get('msg', '')}"""
        ...
        def refresh_system_status():
            if not HAS_MONITOR_AGENT:
                return "<div style='padding: 15px; background: #f8d7da; border-radius: 5px;'>" \
                    "<h3>❌ MonitorAgent 不可用</h3>" \
                    "<p>请先启动 MonitorAgent MCP 服务：</p>" \
                    "<ol>" \
                    "<li><code>python src/mcp_server.py</code></li>" \
                    "<li><code>python src/monitor_agent_mcp.py</code></li>" \
                    "</ol>" \
                    "</div>"
            try:
                ...
                if result["status"] == "success":
                    ...
                    # 根据你的 monitor_agent_logic.py 数据结构调整
                    # 数据格式：{"cpu": {...}, "memory": {...}, "disk": {...}, "top_processes": [...]}
                    ...
                    <div style='padding: 15px; background: #d4edda; border-radius: 5px;'>
                        ...
                        <table border='1' style='border-collapse: collapse; width: 100%; margin-bottom: 15px;'>
                            <tr style='background-color: #f2f2f2;'><th>指标</th><th>值</th><th>状态</th><th>百分比</th></tr>
                            <tr>
                                <td>CPU使用率</td>
                                <td>{cpu_data.get('percent', 0):.1f}%</td>
                                <td>{cpu_data.get('status', '正常')}</td>
                                <td><div style='background: #e0e0e0; width: 100px; height: 20px;'>
                                    <div style='background: #4CAF50; width: {cpu_data.get('percent', 0)}%; height: 100%;'></div>
                                </div></td>
                            </tr>
                            <tr>
                                <td>内存使用</td>
                                <td>{memory_data.get('used_gb', 0):.2f}GB / {memory_data.get('total_gb', 0):.2f}GB</td>
                                <td>{memory_data.get('status', '正常')}</td>
                                <td><div style='background: #e0e0e0; width: 100px; height: 20px;'>
                                    <div style='background: #2196F3; width: {memory_data.get('percent', 0)}%; height: 100%;'></div>
                                </div></td>
                            </tr>
                            <tr>
                                <td>磁盘使用</td>
                                <td>{disk_data.get('used_gb', 0):.2f}GB / {disk_data.get('total_gb', 0):.2f}GB</td>
                                <td>{disk_data.get('status', '正常')}</td>
                                <td><div style='background: #e0e0e0; width: 100px; height: 20px;'>
                                    <div style='background: #FF9800; width: {disk_data.get('percent', 0)}%; height: 100%;'></div>
                                </div></td>
                            </tr>
                        </table>
                    """
                    ...
                    if top_processes:
                        ...
                        for proc in top_processes[:5]:
                            ...
                            html += f"<li><strong>{name}</strong> (PID: {pid}) - CPU: {cpu_percent:.1f}%</li>"
                        html += "</ul>"
                    
                    html += "</div>"
                    return html
                else:
                    return f"<div style='padding: 15px; background: #f8d7da; border-radius: 5px;'>" \
                        f"<h3>❌ 获取系统状态失败</h3>" \
                        f"<p>{result.get('msg', '未知错误')}</p>" \
                        f"</div>"
            except Exception as e:
                return f"<div style='padding: 15px; background: #f8d7da; border-radius: 5px;'>" \
                    f"<h3>❌ 获取系统状态失败</h3>" \
        ...
        def start_monitor_agent_service():
            """启动 MonitorAgent 服务"""
            import subprocess
            import sys
            import os
            ...
            PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            add_log(f"项目根目录: {PROJECT_ROOT}", "info")
            
            # 查找 monitor_agent_mcp.py 文件
            possible_paths = [
                os.path.join(PROJECT_ROOT, "src", "monitor_agent_mcp.py"),
                os.path.join(PROJECT_ROOT, "monitor_agent_mcp.py"),
                os.path.join(PROJECT_ROOT, "src", "monitor_agent_mcp.txt"),
                os.path.join(PROJECT_ROOT, "monitor_agent_mcp.txt"),
            ]
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    script_path = path
                    add_log(f"找到文件: {path}", "success")
                    break
            
            if not script_path:
                # 返回详细的错误信息和帮助
                error_msg = f"""
    ❌ 找不到 monitor_agent_mcp.py 文件

    已搜索的路径:
    {chr(10).join(f'  • {path}' for path in possible_paths)}

    💡 解决方案:
    1. 确认 monitor_agent_mcp.py 文件是否存在
    2. 如果只有 .txt 文件，请重命名:
    mv monitor_agent_mcp.txt monitor_agent_mcp.py
    3. 或者创建新文件:
    cp monitor_agent_mcp.txt src/monitor_agent_mcp.py
    """
                add_log("找不到 MonitorAgent 脚本文件", "error")
                return error_msg
            ...
            if script_path.endswith('.txt'):
                ...
                try:
                    ...
                    add_log(f"已重命名文件: {new_path}", "success")
                except Exception as e:
                    return f"❌ 无法重命名 .txt 文件为 .py: {e}"
            
            try:
                # 先检查 MCP 服务器是否运行
                mcp_script = os.path.join(PROJECT_ROOT, "src", "mcp_server.py")
                if not os.path.exists(mcp_script):
                    return "❌ 找不到 MCP 服务器文件 (mcp_server.py)"
                
                # 启动 MCP 服务器（如果未运行）
                add_log("检查 MCP 服务器状态...", "info")
                ...
                if not mcp_check.stdout.strip():
                    add_log("启动 MCP 服务器...", "info")
                    ...
                    time.sleep(3)  # 等待 MCP 启动
                
                # 启动 MonitorAgent
                add_log(f"启动 MonitorAgent: {script_path}", "info")
                ...
                if result.returncode == 0:
                    output = result.stdout.strip() if result.stdout else "启动成功，无输出"
                    add_log(f"MonitorAgent 启动成功: {output}", "success")
                    
                    # 更新全局状态
                    global HAS_MONITOR_AGENT, monitor_agent
                    try:
                        from monitor_agent_logic import MonitorAgentLogic
                        monitor_agent = MonitorAgentLogic()
                        HAS_MONITOR_AGENT = True
                    except ImportError as e:
                        return f"✅ 服务已启动，但导入模块失败: {e}\n\n输出: {output}"
                    
                    return f"✅ MonitorAgent 启动成功！\n\n服务输出:\n{output}\n\n请点击「刷新系统状态」按钮查看监控信息。"
                else:
                    error_msg = result.stderr.strip() if result.stderr else "未知错误"
                    add_log(f"MonitorAgent 启动失败: {error_msg}", "error")
                    return f"❌ 启动失败 (代码: {result.returncode})\n\n错误信息:\n{error_msg}"
                    
            except subprocess.TimeoutExpired:
                add_log("MonitorAgent 启动超时", "error")
                return "❌ 启动超时，服务可能正在启动中...\n请稍后手动检查服务状态。"
            except Exception as e:
                add_log(f"MonitorAgent 启动异常: {e}", "error")
                return f"❌ 启动异常: {str(e)}"
        
        # 绑定事件
        monitor_start_btn.click(
            fn=start_monitor_agent_service,
            outputs=[monitor_start_output]
        )
        ...
        def refresh_monitor_status():
            ...
            try:
                ...
                monitor_agent = MonitorAgentLogic()
                HAS_MONITOR_AGENT = True
                add_log("MonitorAgent 检测成功", "success")
            except ImportError as e:
                HAS_MONITOR_AGENT = False
                add_log(f"MonitorAgent 检测失败: {e}", "warning")
            ...
            if HAS_MONITOR_AGENT:
                try:
                    ...
                    if result["status"] == "success":
                        ...
                        html = f"""
                        <div style="padding: 15px; background: #d4edda; border-radius: 5px;">
                            <h3 style="color: #155724; margin-top: 0;">✅ 系统状态监控</h3>
                            
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                                <div style="background: white; padding: 10px; border-radius: 5px;">
                                    <h4 style="color: #0c5460; margin: 0 0 5px 0;">CPU 使用率</h4>
                                    <div style="font-size: 24px; font-weight: bold; color: #155724;">
                                        {data.get('cpu', {}).get('percent', 0):.1f}%
                                    </div>
                                    <div style="background: #e9ecef; height: 10px; border-radius: 5px; margin-top: 5px;">
                                        <div style="background: #4CAF50; width: {data.get('cpu', {}).get('percent', 0)}%; height: 100%;"></div>
                                    </div>
                                </div>
                                
                                <div style="background: white; padding: 10px; border-radius: 5px;">
                                    <h4 style="color: #0c5460; margin: 0 0 5px 0;">内存使用</h4>
                                    <div style="font-size: 24px; font-weight: bold; color: #155724;">
                                        {data.get('memory', {}).get('percent', 0):.1f}%
                                    </div>
                                    <div style="background: #e9ecef; height: 10px; border-radius: 5px; margin-top: 5px;">
                                        <div style="background: #17a2b8; width: {data.get('memory', {}).get('percent', 0)}%; height: 100%;"></div>
                                    </div>
                                </div>
                            </div>
                            
                            <div style="background: white; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
                                <h4 style="color: #0c5460; margin: 0 0 5px 0;">磁盘使用</h4>
                                <div style="font-size: 24px; font-weight: bold; color: #155724;">
                                    {data.get('disk', {}).get('percent', 0):.1f}%
                                </div>
                                <div style="background: #e9ecef; height: 10px; border-radius: 5px; margin-top: 5px;">
                                    <div style="background: #ffc107; width: {data.get('disk', {}).get('percent', 0)}%; height: 100%;"></div>
                                </div>
                            </div>
                        ...
                        return f'<div style="padding: 15px; background: #f8d7da; border-radius: 5px;">❌ 获取系统状态失败: {result.get("msg", "未知错误")}</div>'
                except Exception as e:
                    return f'<div style="padding: 15px; background: #f8d7da; border-radius: 5px;">❌ 获取系统状态异常: {str(e)}</div>'
            else:
                return '''
                <div style="padding: 15px; background: #fff3cd; border-radius: 5px; border: 1px solid #ffeaa7;">
                    <h4 style="color: #856404; margin: 0 0 10px 0;">⚠️ MonitorAgent 不可用</h4>
                    <p style="color: #856404; margin: 0;">请先点击「启动 MonitorAgent 服务」按钮启动服务。</p>
                </div>
                '''
        ...
        # 清理进程事件
        def clean_selected_processes(process_name: str):
            if not HAS_MONITOR_AGENT:
                return "❌ MonitorAgent 不可用，请先启动服务"
            
            try:
                name = process_name.strip() if process_name else None
                result = monitor_agent.clean_background_process(name)
                
                if result["status"] == "success":
                    cleaned = result.get("data", {}).get("cleaned_count", 0)
                    if cleaned > 0:
                        return f"✅ 成功清理 {cleaned} 个进程"
                    else:
                        return "✅ 未找到需要清理的进程"
                else:
                    return f"❌ 清理失败: {result.get('msg', '未知错误')}"
            except Exception as e:
                return f"❌ 清理异常: {str(e)}"
        
        process_clean_btn.click(
            fn=clean_selected_processes,
            inputs=[process_clean_name],
            outputs=[process_clean_result]
        )
