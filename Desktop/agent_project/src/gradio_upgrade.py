...
try:
    ...
    from memory_store import list_trajectories, search_trajectories, save_trajectory
...
try:
    ...
    HAS_SYSTEM2 = False
    print("[WARNING] System-2推理模块未找到，将使用关键词匹配模式")
...
def execute_reasoning_plan(reasoning: dict) -> List[str]:
    ...
    for step in execution_plan:
        ...
        try:
            ...
            if agent == "FileAgent":
                if tool == "search_file":
                    # 从action中提取参数（简化版，实际应该从step中获取parameters）
                ...
                if tool == "get_system_status":
                    ...
                    results.append(f"MonitorAgent: {result.get('msg', '执行完成')}")
                elif tool == "clean_processes":
                    process_names = step.get("parameters", {}).get("process_names", [])
                    result = monitor_agent.clean_processes(process_names)
                    results.append(f"MonitorAgent: {result.get('msg', '执行完成')}")
